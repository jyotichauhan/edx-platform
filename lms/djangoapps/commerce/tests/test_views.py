...
class ReceiptViewTests(UserMixin, TestCase):
    ...
    @mock.patch.dict('commerce.views.REASONCODE_MAP', {'99': 'user-message-text'})
    def test_cybersource_message(self, is_user_message_expected, mock_is_user_payment_error):
        """
        Ensure the function is used to display a reason-specific error message or a generic one.
        """
        ...
        generic_message = 'Please wait a few minutes and then try again'
        expected_pattern = 'user-message-text' if is_user_message_expected else generic_message
        unexpected_pattern = 'user-message-text' if not is_user_message_expected else generic_message
        self.assertRegexpMatches(response.content, expected_pattern)
        self.assertNotRegexpMatches(response.content, unexpected_pattern)
